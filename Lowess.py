...
class Lowess:
    ...
    def calculate_weights(self, calculated_distance):
        # calculating weights based on the distances using the specified kernel function
    ...
    def predict(self, xtest, lm=linear_model.Ridge(alpha=0.001)):
        ...
        weights = self.calculate_weights(distance) # computes weights for each distance using specified kernel function
        
        ypred = []  # Initializing empty list to store prediction
        
        for i in range(distance.shape[1]):  # Loop over each test point (each column in the distance matrix)
            ...
            # predictting the value for the ith test point and reshaping for correct dimensions
